(function ($) {
    ...
    $(".ellipsis-text").each(function () {
        ...
        if (text.length > 100) {
            let visibleText = text.substring(0, 93);
            let fadedText = text.substring(93, 100);
            $(this).html(`${visibleText}<span class="fade-text">${fadedText}</span>`);
        }
    });
    ...
    if ($(".datetimepicker").length > 0) {
        $(".datetimepicker").datetimepicker({ format: "YYYY-MM-DD", icons: { up: "fas fa-angle-up", down: "fas fa-angle-down", next: "fas fa-angle-right", previous: "fas fa-angle-left" } });
    }
    ...
    if ($(".datatable").length > 0) {
        $(".datatable").DataTable({ language: { search: '<i class="fas fa-search"></i>', searchPlaceholder: "Search" } });
    }
